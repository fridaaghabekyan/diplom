...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
    ...
    $qTime = mysqli_query($con, "SELECT * FROM `quiz` WHERE `eid` LIKE '$eid'");
    $quizTime = mysqli_fetch_array($qTime)['time'] * 60;
    echo '<input type="hidden" id="timeQuiz" value="'.$quizTime.'">';
    ...
    echo '<form action="update.php?q=quiz&step=2&eid=' . $eid . '&n=' . $sn . '&t=' . $total . '&qid=' . $qid . '" method="POST"  class="form-horizontal" id="questionFrom">
<br />';
...
var seconds =  document.getElementById('timeQuiz').value;
    ...
    if (seconds == 0) {
        ...
        document.getElementById('questionFrom').submit()
